    ...
    public class IntentSender : IIntentSender
    {
        ...
        public async Task<T> SendIntent<T, T2>(T2 args, IntentType type, uint timeBeforeExpiryInSeconds = 30, uint currentTime = 0)
        {
            ...
            catch (TimeMismatchException e)
            {
                ...
                {
                    LogHandler.Error("Unexpected intent payload type: " + intentPayload.GetType());
                }
                ...
                {
                    LogHandler.Warning("Time mismatch detected. Retrying with server time.");
        ...
        {
            LogHandler.Info($"Sending intent: {path} | with payload: {payload}");
