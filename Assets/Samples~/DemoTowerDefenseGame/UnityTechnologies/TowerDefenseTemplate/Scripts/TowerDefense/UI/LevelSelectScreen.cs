...
namespace TowerDefense.UI
{
	...
	public class LevelSelectScreen : SimpleMainMenuPage
	{
		...
		public override void Show()
		{
			...
			_isActive = true;
			ContinuouslyRefreshButtons();
		}

		private async Task ContinuouslyRefreshButtons()
		{
			while (_isActive)
			{
				int buttons = m_Buttons.Count;
				for (int i = 0; i < buttons; i++)
				{
					await m_Buttons[i].GetComponent<LevelSelectButton>().Initialize(m_LevelList[i], mouseScroll);
				}
			}
		}
