...
namespace TowerDefense.UI
{
	...
	public class LevelSelectButton : MonoBehaviour, ISelectHandler
	{
		...
		/// </param>
		public async Task Initialize(LevelItem item, MouseScroll mouseScroll)
		{
			LazyLoad();
		...
		protected async Task HasPlayedState()
		{
			...
			int starsForLevel = await gameManager.GetStarsForLevel(m_Item.id);
			for (int i = 0; i < starsForLevel; i++)
			{
				stars[i].sprite = starAchieved;
			}
			for (int i = starsForLevel; i < stars.Length; i++)
			{
				stars[i].sprite = starNotAchieved;
			}
		}
