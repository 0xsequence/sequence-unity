...
namespace Core.Camera.Editor
{
	...
	public class CameraRigEditor : UnityEditor.Editor
	{
		...
		void OnSceneGUI()
		{
			...
			Vector3 bottom = Handles.FreeMoveHandle(bottomPosition, Quaternion.identity, size, snap,
			                                        Handles.RectangleHandleCap);
			Vector3 top = Handles.FreeMoveHandle(topPosition, Quaternion.identity, size, snap,
			                                     Handles.RectangleHandleCap);
			Vector3 left = Handles.FreeMoveHandle(leftPosition, Quaternion.identity, size, snap,
			                                      Handles.RectangleHandleCap);
			Vector3 right = Handles.FreeMoveHandle(rightPosition, Quaternion.identity, size, snap,
			                                       Handles.RectangleHandleCap);
