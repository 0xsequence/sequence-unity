...
public class SidekickEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (grantRoleFoldout)
        {
            using (new EditorGUILayout.VerticalScope(SectionBoxStyle))
            {
                controller.SelectedContractRole = (ContractRole)EditorGUILayout.EnumPopup("Role", controller.SelectedContractRole);

                switch (controller.SelectedContractRole)
                {
                    case ContractRole.Minter:
                        controller.GrantRoleRole = "0xdfd8f10cabf255ee49ecf3db8b4502eea7eac26c270f6e5f0f28506f6f3fbb55";
                        break;
                    default:
                        throw new ArgumentOutOfRangeException(nameof(controller.SelectedContractRole), "Unsupported role");
                }

                controller.GrantRoleContractAddress = EditorGUILayout.TextField("Contract Address", controller.GrantRoleContractAddress);
                controller.GrantRoleAccount = EditorGUILayout.TextField("Account Address", controller.GrantRoleAccount);
        ...
        if (mintFoldout)
        {
            using (new EditorGUILayout.VerticalScope(SectionBoxStyle))
            {
                ...
                {
                    case ContractType.CurrencyToken:
                        controller.MintERC20ContractAddress = EditorGUILayout.TextField("Currency Address", controller.MintERC20ContractAddress);
                        controller.MintERC20RecipientAddress = EditorGUILayout.TextField("Recipient Address", controller.MintERC20RecipientAddress);
                        controller.MintERC20Amount = EditorGUILayout.TextField("Amount", controller.MintERC20Amount);
                        GUILayout.Space(8);
                    ...
                    case ContractType.NFTContract:
                        controller.SafeMintERC721ContractAddress = EditorGUILayout.TextField("NFT Contract Address", controller.SafeMintERC721ContractAddress);
                        controller.IsBatchMintERC721 = EditorGUILayout.Toggle("Batch Mint", controller.IsBatchMintERC721);
                        ...
                        {
                            controller.SafeMintERC721Recipient = EditorGUILayout.TextField("Recipient Address", controller.SafeMintERC721Recipient);
                            controller.SafeMintERC721TokenId = EditorGUILayout.TextField("Token ID", controller.SafeMintERC721TokenId);
                            GUILayout.Space(8);
                    ...
                    case ContractType.Web3GameItem:
                        controller.MintERC1155ContractAddress = EditorGUILayout.TextField("Item Address", controller.MintERC1155ContractAddress);                
                        controller.IsBatchMintERC1155 = EditorGUILayout.Toggle("Batch Mint", controller.IsBatchMintERC1155);
                        GUILayout.Space(8);
                        ...
                        {
                            controller.MintERC1155RecipientAddress = EditorGUILayout.TextField("Recipient Address", controller.MintERC1155RecipientAddress);
                            controller.MintERC1155Id = EditorGUILayout.TextField("ID", controller.MintERC1155Id);
                            controller.MintERC1155Amount = EditorGUILayout.TextField("Amount", controller.MintERC1155Amount);
                            controller.MintERC1155Data = EditorGUILayout.TextField("Data", controller.MintERC1155Data);
                            GUILayout.Space(8);
        ...
        if (generateFoldout)
        {
            using (new EditorGUILayout.VerticalScope(SectionBoxStyle))
            {
                selectedGenerateType = (ContractType)EditorGUILayout.EnumPopup("Contract Type", selectedGenerateType);
                GUILayout.Space(8);

                switch (selectedGenerateType)
                {
                    case ContractType.CurrencyToken:
                        controller.soContractAddress = EditorGUILayout.TextField("Currency Address", controller.soContractAddress);
                        controller.soTokenDecimals = EditorGUILayout.IntField("Token Decimals", controller.soTokenDecimals);
                        GUILayout.Space(8);

                        if (GUILayout.Button("Create Currency Token ScriptableObject", ButtonStyle, GUILayout.Height(26)))
                        {
                            var asset = CreateInstance<ERC20Scriptable>();
                            asset.contractAddress = controller.soContractAddress;

                            string path = EditorUtility.SaveFilePanelInProject("Save ERC20 ScriptableObject", "NewERC20Scriptable", "asset", "Please enter a file name to save the asset to");
                    ...
                    case ContractType.NFTContract:
                        controller.soContractAddress = EditorGUILayout.TextField("Contract Address", controller.soContractAddress);
                        GUILayout.Space(8);

                        if (GUILayout.Button("Create NFT Contract ScriptableObject", ButtonStyle, GUILayout.Height(26)))
                        {
                            var asset = CreateInstance<ERC721Scriptable>();
                            asset.contractAddress = controller.soContractAddress;

                            string path = EditorUtility.SaveFilePanelInProject("Save ERC721 ScriptableObject", "NewERC721Scriptable", "asset", "Please enter a file name to save the asset to");
                            if (!string.IsNullOrEmpty(path))
                            {
                                AssetDatabase.CreateAsset(asset, path);
                                AssetDatabase.SaveAssets();
                    ...
                    case ContractType.Web3GameItem:
                        controller.soContractAddress = EditorGUILayout.TextField("Item Address", controller.soContractAddress);
                        controller.soSpecifyBurnAddress = EditorGUILayout.Toggle("Specify Burn Address", controller.soSpecifyBurnAddress);

                        if (controller.soSpecifyBurnAddress)
                        {
                            controller.soContractBurnAddress = EditorGUILayout.TextField("Burn Address", controller.soContractBurnAddress);

                        }
                        GUILayout.Space(8);

                        if (GUILayout.Button("Create Web 3 Game Item ScriptableObject", ButtonStyle, GUILayout.Height(26)))
                        {
                            var asset = CreateInstance<ERC1155Scriptable>();
                            asset.contractAddress = controller.soContractAddress;
                            asset.specifyBurnAddress = controller.soSpecifyBurnAddress;

                            string path = EditorUtility.SaveFilePanelInProject("Save ERC1155 ScriptableObject", "NewERC1155Scriptable", "asset", "Please enter a file name to save the asset to");
                            if (!string.IsNullOrEmpty(path))
                            {
                                AssetDatabase.CreateAsset(asset, path);
                                AssetDatabase.SaveAssets();
